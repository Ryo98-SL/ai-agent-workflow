    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[88vh] max-h-[88vh] max-w-5xl flex-col gap-0 overflow-hidden border-2 bg-card p-0 shadow-2xl shadow-black/40">
        <DialogHeader className="shrink-0 border-b border-border px-5 py-4">
          ...
          <DialogDescription>Reusable content for workflow retrieval.</DialogDescription>
        </DialogHeader>
        <div className="grid min-h-0 flex-1 grid-cols-[280px_minmax(0,1fr)] overflow-hidden">
          <aside className="flex min-h-0 flex-col border-r border-border bg-muted/30 p-3">
            ...
            {!isAuthed && <p className="mt-2 shrink-0 text-xs text-muted-foreground">Sign in to create knowledge bases.</p>}
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-1 block text-xs text-muted-foreground">
                      {base.documentCount} docs · {base.characterCount.toLocaleString()} chars
                    </span>
                  </span>
                  {base.readOnly && <span className="rounded bg-muted px-1.5 py-0.5 text-[10px] font-semibold">Example</span>}
                </button>
              ))}
            </div>
          </aside>
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-3 rounded-md border border-dashed border-border p-10 text-center">
      ...
      <p className="text-sm text-muted-foreground">
        {isAuthed ? "No knowledge base yet." : "Sign in to create your own knowledge bases."}
      </p>
      {isAuthed && (
            ...
            <div className="relative">
              ...
              {confirmingDelete && (
                <>
                  ...
                  <div
                    role="dialog"
                    aria-label="Confirm delete knowledge base"
                  ...
                  >
                    <p className="text-sm font-medium text-foreground">Delete this knowledge base?</p>
                    <p className="mt-1 text-xs text-muted-foreground">
                      This permanently removes the knowledge base and all its documents.
                    </p>
